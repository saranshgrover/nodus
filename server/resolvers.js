...
var { WcifModel, VenueModel, PersonModel } = require('./models/wcif/Wcif')
...
WcifTC.addResolver({
	...
	resolve: async ({ args }) => {
		...
		if (!comp) {
			throw new Error(
				`Couldn't find venue with name "${args.venueName}" for competition with ID "${args._id}"`
			)
		}
		// Found comp
		const parsedCompetitors = args.updatedCompetitors
		comp.persons = parsedCompetitors
		const savedComp = await comp.save()
		return savedComp
	},
})
