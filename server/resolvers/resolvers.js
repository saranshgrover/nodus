...
WcifTC.addResolver({
	...
	resolve: async ({ args, info }) => {
		...
		if (!data)
			throw new Error(`Error finding competition with _ID ${args._id}`);
		return data;
	},
});
		...
		persons.forEach(async (person) => {
			...
			if (
				...
				!user.competitions.some(
					(competition) => competition.id === wcif.id
				)
			) {
...
WcifTC.addResolver({
	...
	resolve: async ({ args, info }) => {
		...
		if (!competition)
			throw new Error(
				`No Competition named ${args.competitionId} exists`
			);
		...
		persons.forEach(async (person) => {
			...
			if (
				...
				user.competitions = user.competitions.filter(
					(competition) =>
						competition.competitionId !== args.competitionId
				);
				await user.save();
			}
		});
