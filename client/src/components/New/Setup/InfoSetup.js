...
const UPDATE_COMPETITION_INFO_MUTATION = gql`
    mutation updateWcifInfo($id: String!, $name: String!, $shortName: String!, $competitorLimit: Int!) {
        updateWcifInfo(_id: $id, newName: $name, newShortName: $shortName, newCompetitorLimit: $competitorLimit) {
            id
        }
    }
`
...
export default function InfoSetup({ id, onComplete, handleBack }) {
    ...
    const [updateWcifInfo, mutationOptions] = useMutation(UPDATE_COMPETITION_INFO_MUTATION)
    ...
    return (
        <>
            ...
            >
                <Grid item>
                    <TextField fullWidth value={localData.name} label='Name' name='name' onChange={handleChange} />
                </Grid>
                <Grid item>
                    <TextField
                        fullWidth
