...
export default function Projector({ match }) {
	const [screens, setScreens] = useState(["AGENDA", "COMPETING", "INFO"]);
	...
	useEffect(() => {
		if (!loading && !error) {
			const spotlight =
				data.getTopCompetitors.length > 0 ? ["SPOTLIGHT"] : [];
			const results = data.getOpenRounds.length > 0 ? ["RESULTS"] : [];
			setScreens([...screens, ...spotlight, ...results]);
		}
	}, [data, loading, error]);
